...
public class UpdateToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userType == null || password == null || userName == null || !userType.equalsIgnoreCase("customer")) {
            response.sendRedirect("login.jsp?message=Access Denied ! Please login");
            return;
        }
        ...
        if (stockQty < pQty) {
            String status = "<small>Only " + stockQty + "  "+ product.getProdName()+"are available ! so we can add only " + stockQty + " in your cart";
            DemandDaoImpl demand = new DemandDaoImpl();
            DemandPojo demandPojo = new DemandPojo(userName, prodId, pQty - stockQty);
            boolean flag = demand.addProduct(demandPojo);

            if (flag) {
                status += "<br/> Later we will mail you when " + product.getProdName() + " available</small>";
            }
